...
import { useOrdersColumns } from "@/components/admin/orders/orders-columns";
...
export function OrdersPage() {
  ...
  const statusOptions = [
    { label: t`Čekající`, value: "PENDING" },
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 sm:gap-6">
      <PageHeader
        title={t`Objednávky`}
        subtitle={t`Přehled všech objednávek.`}
      />
      ...
      {ordersQuery.isLoading ? (
        ...
        <DataTable
          ...
          data={data}
          searchKey="id"
          searchPlaceholder={t`Filtrovat objednávky...`}
          filters={filters}
        />
      )}
    </div>
